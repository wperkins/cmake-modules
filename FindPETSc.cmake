  ...
  if (EXISTS "${PETSC_TMP}/include/petscversion.h")
    ...
    if (PETSC_VERSION_RELEASE)
      if ($(PETSC_VERSION_PATCH) GREATER 0)
        set (PETSC_VERSION "${PETSC_VERSION_MAJOR}.${PETSC_VERSION_MINOR}.${PETSC_VERSION_SUBMINOR}p${PETSC_VERSION_PATCH}" CACHE INTERNAL)
      else ()
        set (PETSC_VERSION "${PETSC_VERSION_MAJOR}.${PETSC_VERSION_MINOR}.${PETSC_VERSION_SUBMINOR}" CACHE INTERNAL)
      endif ()
    else ()
      # make dev version compare higher than any patch level of a released version
      set (PETSC_VERSION "${PETSC_VERSION_MAJOR}.${PETSC_VERSION_MINOR}.${PETSC_VERSION_SUBMINOR}.99" CACHE INTERNAL)
    endif ()
  else ()
...
")
  ...
  # If you don't have MPI compiler wrappers, and need to use FindMPI,
  # petsc_test_runs() will often fail because mpi.h cannot be
  # found. If FindMPI is used and succeeds, MPI_C_FOUND will be set,
  # and the found includes and libraries will be used in
  # petsc_test_runs().
  if (MPI_C_FOUND)
